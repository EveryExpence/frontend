...
import { useBalanceTrendDetails, BalanceDataPoint } from "@/hooks/use-balance-trend-details";
...
const getIncomeIcon = (name: string) => {
    ...
    if (n.includes('invest')) return 'cash-multiple';
    if (n.includes('return')) return 'history';
...
const BalanceTrendDetailsScreen: React.FC = () => {
  ...
  const { data, percentageChange, incomeSources, loading, error, refetch } =
          ...
          <>
            ...
            <View style={{ height: 250, width: "100%", marginBottom: 30 }}>
                {data.length > 0 ? (
                    ...
                    <View className="flex-1 items-center justify-center bg-theme-surface rounded-xl">
                        <Text className="text-theme-text opacity-60">No data for this period</Text>
                    </View>
                )}
            </View>
            ...
            <View className="bg-theme-surface rounded-xl p-4">
                ...
                {incomeSources.length === 0 ? (
                    <Text className="text-theme-text opacity-60 italic">No income records found for this period.</Text>
                ) : (
                    incomeSources.map((source, index) => (
                        <View key={source.categoryId} className={`flex-row items-center justify-between py-3 ${index < incomeSources.length - 1 ? 'border-b border-black/5' : ''}`}>
                            <View className="flex-row items-center flex-1">
                                <View className="w-12 h-12 rounded-full bg-theme-tint items-center justify-center">
                                    <MaterialCommunityIcons 
                                        name={getIncomeIcon(source.categoryName)} 
                                        size={24} 
                                        color="white" 
                                    />
                                </View>
                                <Text className="ml-3 text-[20px] font-medium text-theme-text flex-1" numberOfLines={1}>
                                    {source.categoryName}
                                </Text>
                            </View>
