  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.35)', justifyContent: 'center', padding: 16 }}>
        <Pressable onPress={() => {}} style={{ backgroundColor: colors.surface, borderRadius: 12, padding: 16 }}>
          <Text style={{ color: colors.text, fontSize: 18, fontWeight: '600', marginBottom: 8 }}>Delete account</Text>
          <Text style={{ color: colors.icon, marginBottom: 16 }}>Delete {name ?? ''}?</Text>
